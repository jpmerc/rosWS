...
#include <custom_navigation/S_DigitalOut.h>
...
bool setDigitalOut(custom_navigation::S_DigitalOut::Request& request, custom_navigation::S_DigitalOut::Response& response){
...
bool newConfReceived(custom_navigation::S_ADC::Request& request, custom_navigation::S_ADC::Response& response){
	...
	if(request.subscribe){
		...
		if(nbrSubscribers == 1){
			...
			adcPublisher = n->advertise<custom_navigation::M_ADC>(topic_path, request.adcBufferSize);
		}
...
int main(int argc, char* argv[])
{
    ...
    if (attachAdc("/dev/serial/by-id/usb-0683_1490-if00") == 0) {
        ...
        while (ros::ok())
        {
           // ROS_INFO("%s", "Le programe roule1!");
            if(running){
                readData();
               // ROS_INFO("%s", "Le programe roule2!");
            }
            ros::spinOnce();
            loop_rate->sleep();
        }
